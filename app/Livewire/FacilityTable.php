            ...
            Column::make('Actions')
                ->label(
                    function ($row, Column $column) {
                        $show = '<button class="show-btn text-white font-bold p-2 rounded" wire:click="show(' . $row->facility_id . ')">Show</button>';
                        $edit = '<button class="edit-btn text-white font-bold p-2 rounded" wire:click="edit(' . $row->facility_id . ')">Edit</button>';
                        $archive = '<button class="archive-btn text-white font-bold p-2 rounded" onclick="document.getElementById(\'my_modal_' . $row->facility_id . '\').showModal()">Archive</button>
